...
function configureIIS(appName, port, physicalPath, appPoolName) {
  try {
    ...
    log(
      `IIS application '${appName}' configured successfully. Application is available at http://localhost/${appName}/swagger`
    );
  } catch (error) {
...
async function getLatestReleaseUrl() {
  try {
    const url = `https://api.github.com/repos/${Constants.GITHUB_RELEASE_URL}latest`;
...
async function deploy(
  ...
  try {
    ...
    Constants.WriteJsonFile(Constants.CONFIGURATION_FILE_PATH, {
      PORT: port,
      APP_NAME: appName,
    });
  } catch (error) {
