...
async function ensureServiceExistsOrUpdate(appName, port, exePath) {
    ...
    try {
        ...
        try {
            ...
            execSync(`sc.exe config ${appName} binPath= "${exePath}"`, { stdio: 'inherit' });
...
async function deploy(appName, port, useWindowsService) {
    ...
    try {
        ...
        const appPoolName = 'SQLAccountingAppPool'; 
        ...
        const appUrl = 'https://github.com/beehexacorp/sqlaccountapi/releases/download/release-0.0.9/sql-accounting-release-0.0.9-win-x64.zip';
        ...
        if (!!useWindowsService && useWindowsService.toString() === 'true') {
            ...
            configureWindowsService(appName, port, exePath).then(() => {
