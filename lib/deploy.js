...
async function configureWindowsStartup(exePath) {
  ...
  runExe(exePath, workingDirectory);
}
function createShortcut(targetPath, shortcutPath) {
  ...
  const scriptFile = path.join(os.tmpdir(), 'createShortcut.vbs');
  fs.writeFileSync(scriptFile, script);
  
  exec(`cscript //nologo ${scriptFile}`, (err, stdout, stderr) => {
    if (err) {
      console.error("Error creating shortcut:", err);
      return;
    }
    console.log("Shortcut created successfully:", stdout);
    fs.unlink(scriptFile, (err) => {
      if (err) {
        console.error("Error deleting VBScript file:", err);
        return;
      }
      console.log("Temporary VBScript file deleted.");
    });
  }); 
}
function addToStartup(shortcutPath) {
  const startupShortcutPath = path.join(STARTUP_PATH, path.basename(shortcutPath));
  ...
  fs.copyFile(shortcutPath, startupShortcutPath, (err) => {
    ...
    console.log("Shortcut added to startup.");
  });
}
function runExe(exePath, workingDirectory = null) {
  const fullExePath = path.resolve(exePath); // Đảm bảo exePath là đường dẫn đầy đủ

  const process = spawn(fullExePath, [], {
    cwd: workingDirectory || process.cwd(), // Thư mục làm việc
    stdio: 'inherit' // Cho phép hiển thị giao diện chương trình con
  });
...
async function deploy(
  ...
  try {
    if (!!deployMethod && deployMethod.toString() === "WindowsService") port = port ?? 5001;
    port = port ?? 5001;
    log(
...
module.exports = { deploy, downloadFile };
