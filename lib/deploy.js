...
async function configureWindowsService(appName, port, exePath, username, password) {
    try {
        // Ensure service is fully deleted
        return ensureServiceExistsOrUpdate(appName, port, exePath, username, password)
...
async function ensureServiceExistsOrUpdate(appName, port, exePath, username, password) {
...
function ensureAppPoolExists(appPoolName, username, password) {
    try {
        ...
        try {
            ...
            execSync(`powershell -Command "New-WebAppPool -Name '${appPoolName}'; Set-ItemProperty IIS:\\AppPools\\${appPoolName} -name processModel.identityType -value SpecificUser; Set-ItemProperty IIS:\\AppPools\\${appPoolName} -name processModel.userName -value '${username}'; Set-ItemProperty IIS:\\AppPools\\${appPoolName} -name processModel.password -value '${password}'"`, { stdio: 'inherit' });
...
async function deploy(appName, port, useWindowsService, username, password) {
    ...
    try {
        ...
        Constants.WriteJsonFile(Constants.CONFIGURATION_FILE_PATH, {'PORT':port,'APP_NAME':appName})
    } catch (error) {
