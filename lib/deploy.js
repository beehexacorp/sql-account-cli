...
async function getLatestReleaseUrl() {
  try {
    ...
    await WriteJsonFile(CONFIGURATION_FILE_PATH, { API_VERSION: release.tag_name });
...
async function deploy(
  ...
  try {
    ...
    await WriteJsonFile(CONFIGURATION_FILE_PATH, { "APP_DIR": appDir.replace(/\\/g, "/") });
    ...
    if (!!useWindowsService && useWindowsService.toString() === "true") {
      ...
      const exePath = path.join(extractPath, SQL_ACCOUNT_REST_API_APPLICATION_NAME); // Replace with your actual exe name
